      ...
      <el-form-item label="部门负责人" prop="manager">
        ...
        >
          <el-option
            ...
            :value="item.username"
          ></el-option>
          <el-option label="里斯" value="beijing"></el-option>
        </el-select>
      </el-form-item>
...
export default {
  ...
  data() {
    const checkDepName = async (rule, value, callback) => {
      if (this.formData.id) {
        // 编辑
        ...
        // 2. 如果重复 callback(new Error(‘部门重复’))
        isRepeat ? callback(new Error('部门名称重复')) : callback()
        // 3. 如果没重复 callback()
      }
    }
    const checkDepCode = async (rule, value, callback) => {
      const { depts } = await getDeptsApi()
      let isRepeat
      if (this.formData.id) {
        isRepeat = depts
